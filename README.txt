...
For Usage instructions see the unit tests provided with this project.
...
process. The utility comes with 2 pre-build mime detectors, ExtensionMimeDetector and MagicMimeMimeDetector. These are automatically initialised
by the utility but be be de-registered by clients if required. Please see the relevant java doc for each of these mime detectors for a description
...
returned to the client. The collection can contain only one entry for each mime type and can never contain null. These collections are instances of
MimeTypeHashSet that has a backing LinkedHashSet object and implements all the methods of the java.util.Collection and java.util.Set interfaces.
...
We will list the additions/changes to mime-types.properties and magic.mime files on sourceforge as a separated downloads for the project.
This will allow all users to locate these rules easily. We expect that we will add these changes with each new release.
